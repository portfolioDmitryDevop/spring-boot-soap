package com.example.soap.service;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.ws.server.endpoint.annotation.Endpoint;
import org.springframework.ws.server.endpoint.annotation.PayloadRoot;
import org.springframework.ws.server.endpoint.annotation.RequestPayload;
import org.springframework.ws.server.endpoint.annotation.ResponsePayload;

import com.example.soap.repo.CountryRepository;

import io.spring.guides.gs_producing_web_service.GetCountryByCapitalRequest;
import io.spring.guides.gs_producing_web_service.GetCountryByCapitalResponse;
import io.spring.guides.gs_producing_web_service.GetCountryRequest;
import io.spring.guides.gs_producing_web_service.GetCountryResponse;

@Endpoint
public class CountryEndpoint {
	private static final String NAMESPACE_URI = "http://spring.io/guides/gs-producing-web-service";

	private CountryRepository countryRepository;

	@Autowired
	public CountryEndpoint(CountryRepository countryRepository) {
		this.countryRepository = countryRepository;
	}
	
	@PayloadRoot(namespace = NAMESPACE_URI, localPart = "getCountryByCapitalRequest")
	@ResponsePayload
	public GetCountryByCapitalResponse getCountryByCapital(@RequestPayload GetCountryByCapitalRequest request) {
		System.out.println("getCountryByCapital " +  request.getCapital());
		GetCountryByCapitalResponse response = new GetCountryByCapitalResponse();
		response.setCountry(countryRepository.findCountryByCapital(request.getCapital()));

		return response;
	}
	
	@PayloadRoot(namespace = NAMESPACE_URI, localPart = "getCountryRequest")
	@ResponsePayload
	public GetCountryResponse getCountry(@RequestPayload GetCountryRequest request) {
		GetCountryResponse response = new GetCountryResponse();
		response.setCountry(countryRepository.findCountry(request.getName()));

		return response;
	}
	

}
